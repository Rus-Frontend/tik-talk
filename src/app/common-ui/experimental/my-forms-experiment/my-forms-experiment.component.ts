import {AfterViewInit, Component, ElementRef, inject, OnDestroy, Renderer2, signal} from '@angular/core';
...
import mask from '../../../helpers/masks/tel-mask';
...
export class MyFormsExperimentComponent implements OnDestroy, AfterViewInit {
  ...
  paymentForm = new FormGroup({
    ...
    receiverINN: new FormControl<number | null>(null, [Validators.required, Validators.minLength(10), Validators.maxLength(10)]),
    receiverKPP: new FormControl<number | null>(null, [Validators.required, Validators.minLength(9), Validators.maxLength(9)]),
    receiverBIK: new FormControl<string>('', [Validators.required, Validators.minLength(9), Validators.maxLength(9)]),
    receiverBankAcc: new FormControl<string>('', [Validators.required, Validators.minLength(20), Validators.maxLength(20)]),
  ...
  constructor() {
    ...
    this.myMockService.getMe()
        .pipe(takeUntilDestroyed())
        .subscribe(me => {
          this.paymentForm.patchValue(me, {emitEvent: false})
          this.paymentForm.controls.senderBankAcc.patchValue(me.senderBankAcc + ` (${this.splitSum(me.senderBankAccSum)} руб.)`)
        })
    ...
    this.paymentForm.controls.receiverINN.valueChanges
        ...
        .subscribe(value => {
          if (!this.isReceiverPerson() && value?.toString().length === 10) {
            this.myMockService.getReceiverByINN(value)?.pipe()
                .subscribe(receiver => {
  ...
  getPaymentDate() {
    ...
    this.paymentForm.controls.paymentDate.patchValue(this.paymentDate(), {emitEvent: false})
  }
